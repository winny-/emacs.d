...
#+end_src
*** ox-textile
For use with Redmine and friends.

#+begin_src emacs-lisp
  (use-package ox-textile
    :load-path "~/.emacs.d/site-packages/org-textile")
#+end_src
...
*** BASIC support (tinybasic.rkt)

Quick and dirty org-mode support for my BASIC dialect.

#+begin_src emacs-lisp
  (use-package ob-basic)
#+end_src
...
#+BEGIN_SRC emacs-lisp
  (use-package irfc
    :hook (irfc-mode
           . (lambda ()
               (read-only-mode 1))))      ; Make read only.
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (display-battery-mode
   ...
   (if (and (boundp 'battery-status-function)
