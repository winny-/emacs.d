...
** Full ISO 8601
From https://wilkesley.org/~ian/xah/emacs/elisp_datetime.html.  Or if more
flexibility is needed consider e.g. =C-u M-! date -R RET DEL= for RFC 822
(Email) format.
...
#+begin_src emacs-lisp
  (use-package cython-mode
    :ensure t
    :init
    (add-to-list 'auto-mode-alist '("\\.py[xd]\\'" . cython-mode)))
#+end_src
...
#+begin_src emacs-lisp
  (use-package lsp-ui
    ...
    :bind (:map lsp-ui-imenu-mode-map
              ...
              ("p" . previous-line))
    :init
    (defun winny/lsp-ui-imenu-refresh ()
                "Refresh lsp-ui-imenu."
                (when (and (bound-and-true-p lsp-mode)
                           (get-buffer lsp-ui-imenu-buffer-name))
                  (condition-case err (lsp-ui-imenu--refresh)
                    (imenu-unavailable    ; Prevent imenu from soft-locking Emacs.
                     ;; TODO close window that contained lsp-ui-imenu's buffer
                     nil))))
    (add-hook 'buffer-list-update-hook #'winny/lsp-ui-imenu-refresh)
    (add-hook 'lsp-mode-hook #'winny/lsp-ui-imenu-refresh))
#+end_src
...
** Unison profiles
:PROPERTIES:
:ID:       895cfaae-d6b2-47f9-b284-89e61a0d80e1
:END:
...
(use-package unison)
#+END_SRC
** NetHack RC Major mode
Based on [[id:895cfaae-d6b2-47f9-b284-89e61a0d80e1][conf-unison-mode]].  For NetHack RC files.  See [[https://nethackwiki.com/wiki/Options][Options - NetHack Wiki]].
#+begin_src emacs-lisp
  (use-package nethackrc-mode)
#+end_src
** C mode
*** All C-like
...
#+end_src
** Perl
See [[http://xahlee.info/emacs/emacs/emacs_perl_vs_cperl_mode.html][Xah -- Emacs: perl-mode vs cperl-mode]].
...
*** =C-.= enqueues mpv playback

#+BEGIN_SRC emacs-lisp
  (eval-after-load 'dired
    '(progn
       (define-key dired-mode-map (kbd "C-.") 'winny/dired-enqueue-mpv)
       (defun winny/dired-enqueue-mpv ()
         "Enuqueue to MPV"
         (interactive)
         (dired-do-shell-command "mpv-add *" nil (dired-get-marked-files)))))
#+END_SRC
...
#+begin_src emacs-lisp
  (use-package tao-theme
    :ensure t)
#+end_src
** Configure automatic light & dark themes for Emacs
...
** Panic key
Rescue Emacs from soft-locking.  Typically =C-g= or =C-]= spammed enough
rescues Emacs.  Apparently hooking into buffer management can break buffer
switching, therefore break most functionality including the debugger.  The
easiest solution is to prune the errant code and restart Emacs.  Sometimes I
just want to regain control, though 🦾!
#+begin_src emacs-lisp
  (global-set-key (kbd "C-x C-M-\\")
                  (defun winny/rescue-emacs ()
                    "REEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEESCUE EEEEEEMACS"
                    (interactive)
                    (setq post-command-hook nil
                          pre-command-hook nil
                          after-change-functions nil
                          buffer-list-update-hook nil)
                    (message "RESCUE: major hooks nuked for this session.  Run M-x winny/reload-configuration RET to restore.")))
#+end_src
    ...
    (setq org-special-ctrl-a t
          ...
          '(("n" "Note" entry
            ...
            ("j" "Journal Entry" entry
             ...
             :empty-lines 1)
            ("w" "Watch list item" entry
             (file+headline "~/files/notes/watch.org" "Org Capture")
             (file "~/.emacs.d/org-capture-templates/watch.org"))
            ("b" "Blog post" entry
             (file+headline "~/files/writings/blogs.org" "Org Capture")
             (file "~/.emacs.d/org-capture-templates/blog.org"))
            ("t" "Todo list item" entry
             (file+headline "~/files/notes/todo.org" "Inbox")
             (file "~/.emacs.d/org-capture-templates/todo-item.org"))
            ("B" "bench.winny.tech note" entry
             (file "~/p/winny.tech/bench.winny.tech/org/bench.org")
             (file "~/p/winny.tech/bench.winny.tech/org/capture-template.org")))
...
With the following code, one can store manpage links.  See [[info:org#Adding Hyperlink Types][org#Adding Hyperlink
Types]].
#+begin_src elisp
...
Refer to [[https://orgmode.org/worg/library-of-babel.html][The Library of Babel]] and [[https://orgmode.org/worg/org-contrib/babel/languages/index.html#langs][Babel: Languages]].
*** Async execution
#+begin_src elisp
...
Always follow symlinks.  Resolve =buffer-file-name= and =default-directory=.
Shows =magit-status= where my dotfiles git repository is when accessing
=~/bin/*= files directly.
